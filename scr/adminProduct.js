...
function renderProducts() {
  ...
  // Các collection trong Firebase Firesrore
      ...
      .then((querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
          ...
          row.innerHTML = `
                        ...
                        <td>
                        <button class="changeBtn btn btn-sm btn-warning" data-id="${
                          doc.id
                        }" data-collection="${collectionName}">Sửa</button>
                        <button class="deleteBtn btn btn-sm btn-danger" data-id="${
                          doc.id
                        }" data-collection="${collectionName}">Xóa</button>
                        </td>
                    `;
...
productList.addEventListener("click", async (e) => {
  if (e.target.classList.contains("deleteBtn")) {
    ...
    const confirmDelete = confirm(
      "Bạn có chắc chắn muốn xóa tạm thời sản phẩm này không? 🤷‍♂️"
    );
    if (!confirmDelete) {
      return;
    }
    ...
    const confirmChange = confirm(
      "Bạn có chắc chắn muốn thay đổi nội dung của sản phẩm hay không? 🤷‍♂️"
    );
    if (!confirmChange) {
      return;
    }
